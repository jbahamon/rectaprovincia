# Recta Provincia

A small retro-styled platformer made with Godot for LAGS Game Jam 2025 :)

## About

Made solo for the LAGS Game Jam 2025, which lasted 7 days. Games were expected to have some kind of Latin American flavor, plus having "what the eyes don't see, the heart won't feel" as the theme. 

This game is named after the Recta Provincia, which was the name used by a secret organization of warlocks and witches from Chiloé, Chile, for that territory. The name is now commonly used to refer to the organization itself, which was active between the 18th and early 20th century.

The player character has been blinded - the screen is dark except for your sprite, the UI, and graphical representations of the sounds you hear. When you activate one of your magical artifacts you will temporarily be able to see things in a small circle in the screen. As activations are limited, you have to navigate by choosing when to "see". 

## Play It

[Play in your browser or download at itch.io](https://toothykiwi.itch.io/recta-provincia).

## Features

- Localization (though only through system detection, didn't get to put it in settings)

## Screenshots

![The player character, activating his power in the rain, showing the radius of effect](docs/screenshot_1.png)

![New abilities can be unlocked... though I only managed to implement one](docs/screenshot_2.png)

## Credits

### Graphics

Font for dialogue and UI:
  Arbata Compact [Font] (https://lettercore.itch.io/arbata-compact) by Olga Yakovleva (https://lettercore.itch.io/)

Font for titles:
  Font+ [Antiquity Print] (https://ninjikin.itch.io/font-antiquity-script) by SciGho (https://ninjikin.itch.io/)

Font for subheadings:
  Not Jam Old Style 11 (https://not-jam.itch.io/not-jam-old-style-11) by Not Jam (https://not-jam.itch.io/)

Intro cave image:
  Entrance crystal cave (https://commons.wikimedia.org/wiki/File:Entrance_crystal_cave.jpg) by Larry D. Moore (https://en.wikipedia.org/wiki/User:Nv8200pa)

Intro gavel image:
  Legal Gavel & Closed Law Book (https://commons.wikimedia.org/wiki/File:Legal_Gavel_%26_Closed_Law_Book_(27571522123).jpg) by Blogtrepreneur (https://www.flickr.com/people/143601516@N03)

Intro house images:
  Eerie Places (https://beachboogeyman.itch.io/eerie-places) by beachboogeyman (https://beachboogeyman.itch.io/)

Various house interior tiles and objects:
  Game Boy Top-down RPG Fantasy Tileset (https://gumpyfunction.itch.io/game-boy-rpg-fantasy-tileset-free) by Gumpy Function (https://www.gumpyfunction.com/)

Various tiles and objects:
  GB Studio Open Tilesets (https://elvies.itch.io/gb-studio-assets) by Elvies (https://elvies.itch.io/)

House sprite:
  Gameboy Simple RPG Tileset (https://sondanielson.itch.io/gameboy-simple-rpg-tileset) by sonDanielson (https://sondanielson.itch.io/)

Frog sprite:
  GB funky fauna (https://pebonius.itch.io/gb-funky-fauna) by peb (https://pebonius.itch.io/)

UI Panel:
  Free Inventory Sci-Fi (https://elvgames.itch.io/free-sci-fi-inventory) by ElvGames (https://elvgames.com/)

Item icons:
  Items pack (x16) (https://glionox.itch.io/items16) by Glionox (https://glionox.itch.io/)

Bush sprite:
  Gameboy Color Bush 1 (https://dreammix.itch.io/gameboy-bush-1) by Dream Mix (Justin Sink) (https://dreammix.itch.io/)

### Music and Sounds

Title screen BGM:
  Chiptune 8-bit Music Pack (https://firahfabe.itch.io/chiptune-8-bit-music-pack) by FirahFabe (https://firahfabe.com)

Intro BGM, Forest BGM:
  Kastlevania (https://monarkie.itch.io/kastlevania) by Monarkie (Kielen King) (https://kielenkingmusic.com)

Player get hit sound:
  8-bit/Lo-Fi Snare Drum Sound (https://opengameart.org/content/8-bitlo-fi-snare-drum-sound) by Spring Spring (https://spring-enterprises.neocities.org/)

Obtain item sound:
  Flatshot Complete SFX Pack (https://opengameart.org/content/flatshot-complete-sfx-pack) by Rosalila Studio (https://opengameart.org/users/rosalila)

UI accept sound, roar sound:
  200 Free SFX (https://kronbits.itch.io/freesfx) by KronBits (https://kronbits.itch.io/)

Screen transition sound:
  Sweet Sounds - A GB Studio Sound Effect Pack (https://coffeevalenbat.itch.io/sweet-sounds-sfx-pack) by Coffee 'Valen' Bat (https://coffeebat.neocities.org/)

Cutting sound:
  samurai slash (https://freesound.org/people/nekoninja/sounds/370204/) by nekoninja (https://freesound.org/people/nekoninja/)

Steps, shooting, jumping and landing sounds:
  The Essential Retro Video Game Sound Effects Collection [512 sounds] (https://opengameart.org/content/512-sound-effects-8-bit-style) by Juhani Junkala (https://www.youtube.com/@subspaceaudio6850)

Frog sound:
  Ambient Bird, Cricket and Frog (https://opengameart.org/content/ambient-bird-cricket-and-frog) by Blender Foundation (https://apricot.blender.org/)

Challanco sound, enemy get hit sound:
  zipChip Sound Effects Pack (https://zipdisq.itch.io/zipchip-sfx-01) by zipdisqâ„¢ (https://zipdisq.itch.io/)

Health pickup sound:
  Heal - Rpg (https://freesound.org/people/colorsCrimsonTears/sounds/562292/) by colorsCrimsonTears (https://freesound.org/people/colorsCrimsonTears/)

Tue tue sound:
  8-bit Game Sound Effects Collection (https://harvey656.itch.io/8-bit-game-sound-effects-collection) by harvey656 (https://harvey656.itch.io/)

### Inspiration and Research

Reference for mythology and creatures:
  Mitos de Chile: Enciclopedia de seres, apariciones y encantos (https://www.catalonia.cl/libros/mitos-de-chile-enciclopedia-de-seres-apariciones-y-encantos-2/) by Sonia Montecino Aguirre